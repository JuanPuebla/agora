...
<div class="panel panel-info">
    <div class="panel-heading">{gt text="Missatge:"}</div>
        <div class="panel-body">
            <div style="white-space: pre; border-bottom: 1px solid #CCC;">
                {$message}
            </div>
            {if $only_admins eq "1"}
            <div>
    ...
    <tbody>
        {foreach item=service key=id from=$services}
            ...
            <tr class="danger">
                ...
                <td><a target="_blank" href="../{$service->client->clientDNS}/{$service->servicetype->serviceName}">{$service->client->clientName}</a></td>
                <td>{$messages[$id]}</td>
            </tr>
        {/foreach}
    </tbody>
